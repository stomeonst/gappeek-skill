...
商品机会报告先按当前问题和相关前文确定呈现方式，再使用下方字段合同。用户已明确的研究重点优先于默认全球布局；经营背景本身不决定篇幅。

- 聚焦一个平台：约 85% 的有效分析围绕目标平台，约 15% 为其他平台补充。正文先说明该平台的商品细分、实际样本与窗口、匹配组数据、竞争证据及缺口；没有证据的维度直接说明不足。目标平台未进入研究队列时，仍从 `markets` 和 `product_match` 展开事实，不虚构排名或把它加入队列。只有用户明确指定的目标国家才作为主分析范围，没有指定时将返回的国家或区域分别呈现。
- 未指定平台：按服务返回的研究队列、有效证据和缺口组织全球报告，不固定某平台占比。前文仅提到正在经营的平台时仍适用；前文明确的研究目标可以延续，新的全球指令则取消旧的聚焦。
- 明确只看指定平台或市场：全部围绕该范围，不附其他平台推荐。明确比较多个平台时围绕这些平台，按用户指定主次或实际证据展开，不擅自指定主要平台。

比例衡量实际读者看到的分析内容，不要求精确字数。不得通过长篇其他平台表格、附录或辅助材料稀释主平台篇幅，也不凑字数。聚焦模式的其他平台只简要展示本次返回且与问题有关的补充依据及关键缺口；研究队列的相对顺序不变，重复的主平台不再次展开。补充不能声称其他平台已经更好。

主平台没有可用数据时，先明确该平台证据不足，保留具体缺口；禁止把其他平台替换成主结论，或用其他平台填满 85%。没有有效补充时可省略补充。完整服务 JSON 保持原样；面向用户的文档可选择、合并或省略非重点明细，不改写已显示的数值、范围和结论。与解释某项数字有关的样本数、币种、窗口及缺失限制须一并呈现，重复限制可集中说明一次。

显示时可整理金额与比例精度、消除二进制浮点尾差，例如 36.989999999999995 显示为 36.99；不得更换币种、重新计算指标或把微小非零值显示成零。日期时间可换成用户熟悉的时区，同时保留日期和时区，不改变原始观测时刻。

## 全球跨市场报告

`report_kind=cross_platform_comparison` 使用本节。它优先于后面的单平台显示规则。只呈现服务返回的标题匹配摘要、研究队列、候选、可比维度和限制，不用单平台名次或样本总销量重新排序。

简要说明 `query`、`target_scope`、`coverage_basis`、顶层 `observed_at` 和影响当前判断的 `limitations`，再按上节布局展开。`available_markets` 与 `documented_markets` 表示此次已核实目录内的市场范围，不表示全球全部电商。`current_platform` 存在时标为“当前经营背景”，不把它当成研究目标或候选优先级；研究目标来自本次用户意图，不能反向改写报告字段。

`research_plan` 非空时，按其 `purpose` 说明调查目的，按 `selection_policy` 解释服务选取的样本证据范围。全球意图优先呈现 `queue` 中最多三个后续研究对象；平台聚焦意图把其中其他平台压缩为补充，主平台证据仍独立展开。每个平台最多一个，沿用服务顺序；选择依据是匹配样本数量、匹配率和匹配商品有效价格覆盖率，不代表市场机会强弱或蓝海名次。只复述返回的队列，不自行选择、追加或重排，也不自动提交新查询。`queue` 为空时说明目前没有满足调查条件的范围，并解释返回原因；旧报告的 `research_plan` 缺失或为 `null` 时可使用已有候选证据，仍遵守问题要求的主次，不自行生成队列。
...
`markets` 中每一项都是一个平台与国家或区域组合。平台聚焦模式主要展开目标平台，不强制逐项列出所有市场；全球模式可用紧凑覆盖清单按原顺序呈现，详细证据优先解释研究队列所指范围。以下规则约束实际呈现的字段，不要求每次输出全部字段：
...
全球报告的辅助材料按与当前问题的相关性选取，单独显示，不与零售市场合并排名，不要求列出全部商品或帖子。平台聚焦模式下不得借辅助材料挤占主平台分析：
...
全球报告允许展示这些数据缺口、候选限制和核验链接。禁止添加未返回的判断、供应商实现、私有回执、内部费用或计算细节。其他单平台报告继续遵守以下原有合同。
