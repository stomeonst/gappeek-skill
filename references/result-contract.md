...
`ready` 表示已有可交付报告，不保证本次采集全部完成。顶层 `delivery_status=partial` 或任一 `category_research.delivery_status=partial` 时，在正文和 HTML 报告的开头先显示：“已保留已取得数据。本次采集尚未全部完成，部分补充数据未取得。只扣已确认用量；未确认部分不扣。”然后按实际返回数据继续呈现报告，并显示各市场已采集的不同商品数、实际读取页数、停止原因和统计窗口。不能把部分报告称为完全失败、免费、全类目全集或已完成所有数据读取，也不能因此自动重新提交或补调用。

`delivery_status=complete` 仅表示本次运行完成，不能推出完整市场覆盖；缺少该字段的历史报告沿用已有范围，不补写采集状态。`collection_interrupted` 表示后续采集未完成，`supplementary_data_unavailable` 表示部分补充数据未取得，`network_interrupted` 表示后续数据读取连接中断，`no_data_receipt_missing` 表示后续未返回可确认数据。客户可见内容仅使用这些简短解释，不输出原始代码、服务来源名或内部费用。实际积分以账号账本为准，客户端不计算或推测未返回的扣分金额。
...
商品名称分页路径显示“关键词搜索结果”，保留实际页数与去重数量。`category_research.products[].query_match` 为 matched、uncertain、excluded 时分别显示标题匹配、待核对、不匹配；不推定实物属性或全类目覆盖。
